...
import { runPongTwoGame, wsTwo } from "../router/pongTwo.js";
import { runPongFourGame, wsFour } from "../router/pongFour.js";
import { runPongCoopGame } from "../router/pongCoop.js";
...
export default class GamePage extends HTMLElement {
  ...
  connectedCallback() {
    ...
    let match_id = null;
    game_header.textContent = "PING PONG";
    
    if (game_type_query === "two")
      game_type.textContent = "Two Players";
    else if (game_type_query === "four") 
      game_type.textContent = "Four Players";
    else if (game_type_query === "ai")
      game_type.textContent = "AI";
    else if (game_type_query === "coop") 
      game_type.textContent = "Co-op";
    if (game_match_query) 
      match_id = Number(game_match_query);
    ...
    if (game_header.textContent === "PING PONG") {
      if (game_type_query === "two")
        runPongTwoGame(canvas, ctx, match_id);
      else if (game_type_query === "four")
        runPongFourGame(canvas, ctx);
      else if (game_type_query === "coop")
        runPongCoopGame(canvas, ctx);
    }
  }
}
