...
const setImage = (pong_players_elem, avatar, username) => {
...
export const runPongFourGame = (canvas, ctx, Ball, Paddle, keys) => {
  canvas.width = 1300;
  canvas.height = 1300;
  wsFour = new WebSocket(`wss://${window.ft_transcendence_host}/ws/matchmaking/4/`);
  
  const ball = Ball([canvas.width / 2, canvas.height / 2], 20);
  const paddle1 = Paddle([60, canvas.height / 2 - 100], [40, 200]);
  const paddle2 = Paddle([canvas.width - 100, canvas.height / 2 - 100], [40, 200]);
  const paddle3 = Paddle([canvas.width / 2 - 100, 60], [200, 40]);
  const paddle4 = Paddle([canvas.width / 2 - 100, canvas.height - 100], [200, 40]);
  ...
  if (FourPressKey)
    window.addEventListener("keydown", (e) => {
      if (e.keyCode in keys && wsFour.readyState !== WebSocket.CLOSED)
        wsFour.send(keys[e.keyCode]);
    });
  
  FourPressKey = false;
};

const gameLoop = (canvas, ctx, ball, paddle1, paddle2, paddle3, paddle4, pos) => {
  ctx.clearRect(0, 0, canvas.width, canvas.height);
  ball.render(ctx);

  const renderPaddle = (paddle, posPaddle, paddlePos) => {
    if (!paddle.eliminated) {
      if (pos === posPaddle) paddle.render(ctx, "#f8ec90");
      else paddle.render(ctx);
    }
  };

  renderPaddle(paddle1, 1);
  renderPaddle(paddle2, 2);
  renderPaddle(paddle3, 3);
  renderPaddle(paddle4, 4);
};
